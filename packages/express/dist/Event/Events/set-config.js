...
function initModels(models) {
    Object.keys(models).map(key => {
        const modelBuilder = new _adminInterfaceMnCore.ModelBuilder(models[key].default || models[key]);
        // Add model in registry
        _adminInterfaceMnCore.Registry.getRepository('Model').set(key, modelBuilder.setKey(key).getModel());
        return modelBuilder;
    }).map((modelBuilder
...
function initPages(pages) {
    Object.keys(pages).forEach(key => {
        ...
        _adminInterfaceMnCore.Registry.getRepository('Page').set(key, page.setKey(key));
    });
}

function initMenus(menus) {
    Object.keys(menus).forEach(key => {
        _adminInterfaceMnCore.Registry.getRepository('Menu').set(key, (0, _adminInterfaceMnCore.menuParser)(menus[key], key));
    });
}

function initFieldTypes(fieldTypes) {
    Object.keys(fieldTypes).map(key => _adminInterfaceMnCore.Registry.getRepository('FieldType').set(key, fieldTypes[key]));
}

function initConfig(config) {
    Object.keys(config).forEach(key => _adminInterfaceMnCore.Registry.getRepository('Config').set(key, config[key]));
}
...
function initWidgets(widgets) {
    Object.keys(widgets).forEach(key => {
        ...
        _adminInterfaceMnCore.Registry.getRepository('Widget').set(key, widget);
    });
}
